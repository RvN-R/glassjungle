...
I did a great deal of research into the clients demographic by visiting sites of their competitors to see how they connect with the demographic. From the research I found that the demographic wanted to connect with nature, and recreate that in there living space. I felt that using a high resolution image of a rainforest as the body's background throughout the site would tap into that need of connection to nature. I then used a colour palette across the site that complimented that background image. 

* **We need a platform that succinctly explains who we are and the function of the company.**
The home page is the first page a user will arrive at, in the centre of that home page is an "About Us" section that explains succinctly the function of the clients company. 
...
* There are a number of ways for the user to register an account on the site. I've included a link on the home page, which main function it to entice first time visitors of the site to register. If a returning vistor over looks this link during their first visit, then they can utilise it in order to register an account. In addition should a user find their way onto the log in page and hasn't registered, then there is a link to the register page. Thirdly should a user click the "My Account" link on the top right hand corner of the navigation bar and are not logged in then the drop down has a link called "Register".
...
* Assuming the user is logged in they can leave a review on a product relatively easily. Quickest route to leaving a review is  clicking one of the three large buttons on the home page, that have the titles "Empty Terrariums", "Full Terrariums" and "Terrarium Accessories". By clicking one of these call to action buttons the user will be transported to the product category page associated with that button. They can click on the image of one of the products that catches their interest, transporting them to that products product detail page. Once there they can scroll down to the bottom of the page to the "Customer Review" section. They can click the "LEAVE A REVIEW" button. Providing they haven't already left a review they will be able to fill out the review form. Providing they have filled out the form correctly once they click "ADD REVIEW" then the review is uploaded to that product.  
...
* Hover over each button and confirm that the hover function is working properly.
* At this point you should log into your account, for update on how to do this scroll down below to **Testing Sign Up Function** or **Testing Sign In Function**. 
...
* Click the "SIGN UP" call to action button, the form won't submit. Instead the boarder to the email field will go red with an error icon. An error message will appear underneath the email field saying "A user is already registered with this e-mail address".  

### Login
####  Testing Links to Sign In Page
* Load the Website. 
* Make sure you are logged out. If you are unsure how to do that scroll down to **Testing Sign Out Function** and follow these steps. 
* Right click and select inspection.
...
* Navigate to the Sign In page. If you have issues doing this make sure you are logged out. If you are unsure how to do that scroll down to **Testing Sign Out Function** and follow those steps. 
* Right click and select inspection.
* Toggle between different device screen sizes and confirm that text and buttons remain visible and unrestricted.
...
* Load the Website. 
* Make sure you are logged out. If you are unsure how to do that scroll to **Testing Sign Out Function** and follow these steps. 
...
## Known Bugs 
To the average user this isn't a bug, however its something you may notice when testing the site, and is worth explaining. When a user try's to upload an image larger than 2MB using the "Create Post" form on the "Share Your Builds" section of the site, the form won't submit. Instead an error message will appear prompting them to upload a smaller image. This form of validation is prevalent whether a user attempts to upload a large image when creating a post, or amending an existing post on the "Share Your Builds" section of the site. However, the messages they receive aren't identical. Below is a screen shot of each message: 
